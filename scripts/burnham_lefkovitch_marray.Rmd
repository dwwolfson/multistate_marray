...
### Live-dead multistate m-arrays with variable duration sub-adult stages

This document demonstrates potential model structures for estimating survival and encounter probabilities using joint live-dead data for 3 age-classes where at least some individuals are of unknown stage at time of marking. The original motivation was for Giant Canada Geese that can only be differentiated as hatch-year (HY) and after-hatch-year (AHY) at time of banding. Geese and other "k-selected" species have deferred breeding and evidence suggests that models with 3 or more age-classes are necessary to accurately describe vital rates. However, simulation modeling indicates that if AHY individuals cannot be aged at time of capture, survival of HY and SY individuals and dead encounter probabilities of SY individuals will be biased, hence the motivation to develop efficient and reliable models using joint-encounter data. The size and complexity of goose data sets (thousands of releases, decades of data) precludes use of individual state-space approaches, hence the secondary objective of extending the multi-state m-array approach developed for cormorants in Schaub & Kery (2021) to additional model structures. 

An earlier version of this model (burnham_marray.Rmd) dealt with 3 age classes where all yearlings (SY in banding terminology) transition to older adults (ASY) if they survive the next year. This model allows individuals to remain in a sub-adult stage for variable lengths of time by adopting a Lefkovitch projection where some SY individuals graduate (g) to the next stage, but others (1-g) persist as subadults. The easiest way to think of this is a finite mixture where survival of subadults is partitioned into two components: (1-g)s that persist and g(s) that graduate to the next stage.
...
### Scenario 2: Burnham formulation with Lefkovitch projection
...
* Survival (s) varies by year and age class (state 1 = hatch year HY, state 2 = subadult/nonbreeder, and state 3 = adult breeder), but only 2 age classes can be recognized at time of banding (HY and state 4 AHY: which are pooled subadults & adults). 
* The parameter pi estimates the proportion of AHY newly marked each year that are subadult at time of banding (unlike the age-specific model, not all of this group will transition to the third age group if they survive). pi is allowed to vary by year to accommodate annual variation in the proportion of the AHY population that is comprising on nonbreeding subadults.
* The graduation parameter g represents the proportion of surviving subadults that transition to become breeding adults; the (1-g) proportion of survivors persist as subadults for another year.
* Fidelity (F) is the annual proportion of the surviving population that returns to the breeding colony each year, with (1-F) permanently dispersing to other colonies where they are no longer available for recapture/resighting during the annual summer banding period. If individuals survive and disperse, they transition to state 6 (dispersed subadult) or state 7 (dispersed adult), where they are still vulnerable to hunting and may be encountered dead, but can no longer be observed alive (p = 0). F also varies by year and age class. Survival, dead recovery and graduation probabilities are assumed to remain the same for dispersing (1-F) versus non-dispersing (F) individuals.  
...
* Finally, if individuals survive and return to the breeding colony, they can be observed alive in states 2 or 3 with probability (p). Indexing for this parameter is based on stge and year when observed, not age and year when released, hence a HY individual released in year 4 and returning to the colony in year 5 will have encounter probability p[2,5], not p[1,4]. Priors for p are generated for HY individuals although not used in this model, but probability of entry could be useful for estimating population sizes at time of capture, including ratios of population size (e.g. N3/(N2+N3) = breeding propensity).   
...
true_parms <- rbind(F, p, r, s, pi, g)
write.csv(true_parms, "true_parms2.csv")
true_hypers <- rbind(F.mu, F.sd, r.mu, r.sd, p.mu, p.sd, s.mu, s.sd, 
                     rep(pi.mu,3), rep(pi.sd,3), rep(g.mu,3), rep(g.sd,3)) # tripling to keep dimensions similar
write.csv(true_hypers, "true_hypers2.csv")
...
plot(s[1,1:19], out2$mean$s[1,1:19], xlim = c(0.3,1), ylim = c(0.3,1), ylab = "Estimated survival", xlab = "True survival")
segments(s[1,1:19], out2$q2.5$s[1,1:19], s[1,1:19], out2$q97.5$s[1,1:19])
points(s[2,1:19], out2$mean$s[2,1:19], col = "red")
segments(s[2,1:19], out2$q2.5$s[2,1:19], s[2,1:19], out2$q97.5$s[2,1:19], col = "red")
points(s[3,1:19], out2$mean$s[3,1:19], col = "blue")
segments(s[3,1:19], out2$q2.5$s[3,1:19], s[3,1:19], out2$q97.5$s[3,1:19], col = "blue")
abline(a = 0, b = 1)
legend(0.5, 1, legend = c("HY","SY","ASY"), text.col = c("black","red","blue"), bty = 'n')

plot(F[1,1:19], out2$mean$F[1,1:19], xlim = c(0.5,1), ylim = c(0.5,1), ylab = "Estimated fidelity", xlab = "True fidelity")
segments(F[1,1:19], out2$q2.5$F[1,1:19], F[1,1:19], out2$q97.5$F[1,1:19])
points(F[2,1:19], out2$mean$F[2,1:19], col = "red")
segments(F[2,1:19], out2$q2.5$F[2,1:19], F[2,1:19], out2$q97.5$F[2,1:19], col = "red")
points(F[3,1:19], out2$mean$F[3,1:19], col = "blue")
segments(F[3,1:19], out2$q2.5$F[3,1:19], F[3,1:19], out2$q97.5$F[3,1:19], col = "blue")
abline(a = 0, b = 1)
legend(0, 1, legend = c("HY","SY","ASY"), text.col = c("black","red","blue"), bty = 'n')

plot(r[1,1:19], out2$mean$r[1,1:19], xlim = c(0,0.6), ylim = c(0,0.6), ylab = "Estimated recovery", xlab = "True recovery")
segments(r[1,1:19], out2$q2.5$r[1,1:19], r[1,1:19], out2$q97.5$r[1,1:19])
points(r[2,1:19], out2$mean$r[2,1:19], col = "red")
segments(r[2,1:19], out2$q2.5$r[2,1:19], r[2,1:19], out2$q97.5$r[2,1:19], col = "red")
points(r[3,1:19], out2$mean$r[3,1:19], col = "blue")
segments(r[3,1:19], out2$q2.5$r[3,1:19], r[3,1:19], out2$q97.5$r[3,1:19], col = "blue")
abline(a = 0, b = 1)
legend(0, 1, legend = c("HY","SY","ASY"), text.col = c("black","red","blue"), bty = 'n')

# for simulation, p indexed to recapture year, not release year
plot(p[2,2:20], out2$mean$p[2,1:19], xlim = c(0,0.6), ylim = c(0,0.6), ylab = "Estimated recapture", xlab = "True recapture", col = "red")
segments(p[2,2:20], out2$q2.5$p[2,1:19], p[2,2:20], out2$q97.5$p[2,1:19], col = "red")
points(p[3,2:20], out2$mean$p[3,1:19], col = "blue")
segments(p[3,2:20], out2$q2.5$p[3,1:19], p[3,2:20], out2$q97.5$p[3,1:19], col = "blue")
abline(a = 0, b = 1)
legend(0, 1, legend = c("HY","SY","ASY"), text.col = c("black","red","blue"), bty = 'n')

par(mfrow = c(1,2))
plot(pi[1:19], out2$mean$pi[1:19], xlim = c(0,0.6), ylim = c(0,0.6), ylab = "Estimated SY proportion", xlab = "True SY proportion")
segments(pi[1:19], out2$q2.5$pi[1:19], pi[1:19], out2$q97.5$pi[1:19])
abline(a = 0, b = 1)
plot(g[1:19], out2$mean$g[1:19], xlim = c(0,1), ylim = c(0,1), ylab = "Estimated graduationtion", 
     xlab = "True graduation")
segments(g[1:19], out2$q2.5$g[1:19], g[1:19], out2$q97.5$g[1:19])
abline(a = 0, b = 1)

par(mfrow = c(1,2))
true_hyper_mu <- c(s.mu, F.mu, r.mu, p.mu[2:3], pi.mu, g.mu)
est_hyper_mu <- c(out2$mean$s.mu, out2$mean$F.mu,  out2$mean$r.mu, out2$mean$p.mu[2:3], out2$mean$pi.mu, out2$mean$g.mu)
est_hyper_lci <- c(out2$q2.5$s.mu, out2$q2.5$F.mu,  out2$q2.5$r.mu, out2$q2.5$p.mu[2:3], out2$q2.5$pi.mu, out2$q2.5$g.mu)
est_hyper_uci <- c(out2$q97.5$s.mu, out2$q97.5$F.mu,  out2$q97.5$r.mu, out2$q97.5$p.mu[2:3], out2$q97.5$pi.mu, out2$q97.5$g.mu)

plot(est_hyper_mu, pch = 19, col = "blue", cex = 0.8, main = "Logit-scale hyper means", xlab = "True", ylab = "Estimated")
segments(1:13, est_hyper_lci, 1:13, est_hyper_uci)
points(true_hyper_mu, col = "red")
legend(0, 0, legend = c("Estimates", "Truth"), text.col = c("blue", "red"), bty = "n")

true_hyper_sd <- c(s.sd, F.sd, r.sd, p.sd[2:3], pi.sd, g.sd)
est_hyper_sd <- c(out2$mean$s.sd, out2$mean$F.sd,  out2$mean$r.sd, out2$mean$p.sd[2:3], out2$mean$pi.sd, out2$mean$g.sd)
est_hyper_lci <- c(out2$q2.5$s.sd, out2$q2.5$F.sd,  out2$q2.5$r.sd, out2$q2.5$p.sd[2:3], out2$q2.5$pi.sd, out2$q2.5$g.sd)
est_hyper_uci <- c(out2$q97.5$s.sd, out2$q97.5$F.sd,  out2$q97.5$r.sd, out2$q97.5$p.sd[2:3], out2$q97.5$pi.sd, out2$q97.5$g.sd)

plot(est_hyper_sd, pch = 19, col = "blue", cex = 0.8, main = "Logit-scale hyper SDs", xlab = "True", ylab = "Estimated", ylim = c(0,2))
segments(1:13, est_hyper_lci, 1:13, est_hyper_uci)
points(true_hyper_sd, col = "red")
legend(0, 0, legend = c("Estimates", "Truth"), text.col = c("blue", "red"), bty = "n")
