...
# This script will be for model 1
...
# Model 1
# don't need to write file to disk again
...
#    psi[1,t,6] <- s[1,t]*(1-F[1,t]) # survive, emigrate (unobservable)
#    psi[1,t,7] <- 0
#    psi[1,t,8] <- (1-s[1,t])*(1-r[1,t]) # die, not recovered (unobservable)
#    
#   # departure SY resident
#    psi[2,t,1] <- 0
#    psi[2,t,2] <- 0
#    psi[2,t,3] <- s[2,t]*F[2,t] # survive to ASY, remain
#    psi[2,t,4] <- 0
#    psi[2,t,5] <- (1-s[2,t])*r[2,t] # die, recovered 
#    psi[2,t,6] <- 0
#    psi[2,t,7] <- s[2,t]*(1-F[2,t]) # survive, emigrate (unobservable)
#    psi[2,t,8] <- (1-s[2,t])*(1-r[2,t]) # die, not recovered (unobservable)
#    
#   # departure ASY resident
#    psi[3,t,1] <- 0
#    psi[3,t,2] <- 0
#    psi[3,t,3] <- s[3,t]*F[3,t] # survive, remain
#    psi[3,t,4] <- 0
#    psi[3,t,5] <- (1-s[3,t])*r[3,t]  # die, recovered
#    psi[3,t,6] <- 0
#    psi[3,t,7] <- s[3,t]*(1-F[3,t])  # survive, emigrate (unobservable)
#    psi[3,t,8] <- (1-s[3,t])*(1-r[3,t])  # die, not recovered (unobservable)
#    
#   # departure AHY resident (all become ASY, but uncertain departure states)
#    psi[4,t,1] <- 0
#    psi[4,t,2] <- 0
#    psi[4,t,3] <- pi[t]*s[2,t]*F[2,t] + (1-pi[t])*s[3,t]*F[3,t] # SY portion survive to ASY and remain; plus ASY survive and remain
#    psi[4,t,4] <- 0
#    psi[4,t,5] <- pi[t]*(1-s[2,t])*r[2,t] + (1-pi[t])*(1-s[3,t])*r[3,t] # SY portion die and are recovered
#    psi[4,t,6] <- 0
#    psi[4,t,7] <- pi[t]*s[2,t]*(1-F[2,t]) + (1-pi[t])*s[3,t]*(1-F[3,t]) # SY portion survive, emigrate; plus ASY survive, emigrate (unobservable)
#    psi[4,t,8] <- pi[t]*(1-s[2,t])*(1-r[2,t]) + (1-pi[t])*(1-s[3,t])*(1-r[3,t]) # SY portion die, not recovered; plus ASY the same (unobservable)
#    
#   # recently dead (simplified coding, all transition to long dead) 
#    psi[5,t,1:8] <- c(0, 0, 0, 0, 0, 0, 0, 1) # if you died last round, you go to the deep freeze for the rest of the years
#    
#   # departure SY emigrant; once you leave you can't come back
#    psi[6,t,1] <- 0
#    psi[6,t,2] <- 0
#    psi[6,t,3] <- 0
#    psi[6,t,4] <- 0
#    psi[6,t,5] <- (1-s[2,t])*r[2,t] # SY dies, is recovered
#    psi[6,t,6] <- 0
#    psi[6,t,7] <- s[2,t]           # SY survives, (unobservable)
#    psi[6,t,8] <- (1-s[2,t])*(1-r[2,t])  # SY dies, not recovered (unobservable)
#    
#   # departure ASY emigrant; once you leave you can't come back
#    psi[7,t,1] <- 0
#    psi[7,t,2] <- 0
#    psi[7,t,3] <- 0
#    psi[7,t,4] <- 0
#    psi[7,t,5] <- (1-s[3,t])*r[3,t] # ASY dies, is recovered
#    psi[7,t,6] <- 0
#    psi[7,t,7] <- s[3,t]             # ASY survives, (unobservable)
#    psi[7,t,8] <- (1-s[3,t])*(1-r[3,t]) # ASY dies, not recovered (unobservable)
#    
#   # already dead (simplified coding) 
#    psi[8,t,1:8] <- c(0, 0, 0, 0, 0, 0, 0, 1) # still dead/unobserved
# 
#  # observation matrix 
...
param_list<-list(F, p, r, s, pi,
                 F.mu, F.sd, r.mu, r.sd, p.mu, p.sd, s.mu, s.sd, rep(pi.mu,3), rep(pi.sd, 3))
                
names(param_list)<-c("F", "p", "r", "s", "pi",
                     "F.mu", "F.sd", "r.mu", "r.sd", "p.mu", "p.sd", "s.mu", "s.sd", "pi.mu", "pi.sd")

# save to file
saveRDS(param_list, here(glue::glue("MSI/simulations_head/Nov_2025/output/true_params/param_set_simulation_run_{xx}.rds")))
...
##############################
# Create capture history and convert to m-array
################################
...
write_csv(as.data.frame(truth), here(glue::glue("MSI/simulations_head/Nov_2025/output/sim_datasets/simulated_truth_ch_simulation_run_{xx}.csv")))

# save m-array to file
write_csv(as.data.frame(marr), here(glue::glue("MSI/simulations_head/Nov_2025/output/sim_datasets/simulated_marray_simulation_run_{xx}.csv")))
...
# MCMC settings
ni <- 7500; nb <- 2500; nt <- 1; nc <- 4; na <- 1000 
...
out <- jagsUI(jags.data, inits = NULL, parms, "burnham_marray_Nov_2025.txt", 
...
cat("Simulation run number ", xx, "took ", run_min, " minutes to fit the jags model.", "\n",
    file=here("MSI/simulations_head/Nov_2025/output/model_results/model_runtimes.txt"), append=T)
...
write_csv(df, here(glue::glue("MSI/simulations_head/Nov_2025/output/model_results/model_summary_simulation_run_{xx}.csv")))
